...
class ItemSensor(PollingSensor):
    ...
    def poll(self):
        ...
        items = self.account.root.get_folder_by_name(self.sensor_folder).filter(is__read=False).filter(start__gt=start_date).values('item_id', 'subject', 'body', 'datetime_received')
